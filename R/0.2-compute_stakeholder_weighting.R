...
dist_matrix <- vegdist(stakeholder_weighting[-c(1)], method = "euclidean")
...
# pairwise.adonis
Y <- stakeholder_weighting[-1] %>% as.data.frame()
pairwise.adonis2(Y ~ stakeholder, method = "euc",
                 data = stakeholder_weighting)
...
alluvial_plot <- stakeholder_weighting %>% 
  ...
  ggplot(aes(axis1 = stakeholder, axis2 = variable, y = wts)) +
