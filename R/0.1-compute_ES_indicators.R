...
my_data <- es_indicator %>% 
  select(forage_prod, forage_quality, SR, soil_C_stock, csr)
cor_mat <-  my_data %>% cor()
p_mat <- psych::corr.test(my_data)$p

# Create a function to combine lower (cor) and upper (p-value) triangles
lower_tri <- cor_mat
upper_tri <- p_mat

# Replace upper triangle with p-values, formatted
result <- cor_mat
result[upper.tri(result)] <- round(upper_tri[upper.tri(upper_tri)], 4)

# Replace lower triangle with correlations (rounded to 3 decimals)
result[lower.tri(result)] <- round(lower_tri[lower.tri(lower_tri)], 3)

# Add significance stars to upper triangle (optional)
signif_stars <- ifelse(p_mat < 0.001, "***",
                       ifelse(p_mat < 0.01, "**",
                              ifelse(p_mat < 0.05, "*", "")))

# Combine p-value and stars in upper triangle
upper_with_stars <- matrix("", nrow = nrow(p_mat), ncol = ncol(p_mat))
upper_with_stars[upper.tri(upper_with_stars)] <- 
  paste0(format(p_mat[upper.tri(p_mat)], digits = 3, scientific = FALSE), 
         ifelse(signif_stars[upper.tri(signif_stars)] != "", 
                paste0(" ", signif_stars[upper.tri(signif_stars)]), ""))

# Final matrix with correlations below and p-values + stars above
final_matrix <- cor_mat
final_matrix[lower.tri(final_matrix)] <- round(lower_tri[lower.tri(lower_tri)], 3)
final_matrix[upper.tri(final_matrix)] <- upper_with_stars[upper.tri(upper_with_stars)]

# Set diagonal to NA or variable names
diag(final_matrix) <- "--"

# Convert to data frame for nice printing
final_table <- as.data.frame(final_matrix)
rownames(final_table) <- colnames(my_data)
colnames(final_table) <- colnames(my_data)

# Print the table
print(final_table, na.print = "")
